...
for src in "${SOURCES[@]}"; do
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        clang++ -O3 -fsanitize=address -fno-omit-frame-pointer -g -march=native -ffast-math -std=c++17 -arch arm64 -I./include -c "$src" -o "$obj"
    else
...
if [[ "$(uname)" == "Darwin" ]]; then
    clang++ -dynamiclib -arch arm64 -fsanitize=address -I./include -o ${OUTPUT_DIR}/libnn.dylib "${OBJS[@]}" -framework Metal -framework Foundation -Wl,-rpath,/usr/local/lib
else
